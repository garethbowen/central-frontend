...
<template>
  <template v-if="!loadingState">
    <Dialog modal v-if="errorCode === 404" :draggable="false" :closable="false" :visible="true">
      <template #header>
        {{ t('form_not_found.title') }}
      </template>
      <template #default>
        {{ t('form_not_found.body') }}
      </template>
    </Dialog>
    <Dialog modal v-else-if="errorCode" :draggable="false" :closable="false" :visible="true">
      <template #header>
        {{ t('error_not_problem.title') }}
      </template>
      <template #default>
        <p>{{ t('error_not_problem.body') }}</p>
        <p class="error-code">{{ t('error_not_problem.status', { status: errorCode }) }}</p>
      </template>
    </Dialog>
